...
//  FlappyBird
//
//  Created by xiang zhang on 14-2-19.
...
static NSInteger const kVerticalGap = 100;
...
//        [self addChild:_bird];
        ...
        for(int i=0;i<2+self.frame.size.width/(groundTexture.size.width*2);++i){// setScale 2.0 so /size.width*2
...
//            [self addChild:sprite];
        ...
        SKAction* moveSkyLineSpriteForerver = [SKAction repeatActionForever:[SKAction sequence:@[moveSkyLineSprite,resetSkyLineSprite]]];
        ...
        for (int i=0; i<2+self.frame.size.width/(skyLineTexture.size.width*2);++i) {
            ...
            [sprite runAction:moveSkyLineSpriteForerver];
...
-(void) spawnPipes{
    ...
    [pipe2 setScale:2.0];
    pipe2.position=CGPointMake(0, y+pipe1.size.height+kVerticalGap);
...
-(void)didBeginContact:(SKPhysicsContact *)contact{
        ...
        if(_moving.speed>0){
            ...
            }], [SKAction waitForDuration:0.05]]] count:4],[SKAction runBlock:^{_canRestart=YES;}]]] withKey:@"flash"];
            }
    }
}
